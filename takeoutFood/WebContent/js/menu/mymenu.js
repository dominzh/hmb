...
function Q6MenuBar(objName) {
	...
	this.colors[0] = {
		...
		itemBgColor:"#fff",
		itemBorderColor:"#D6C1BE",
		itemBgMoveColor:"#EED2EE",
		itemMoveColor:"red",
		itemColor:"#D1D1D1",
		menuBarColor:"#FFF",
	...
	this.colors[1] = {
			...
			menuBorderColor:"#BBB",
			itemBgColor:"#fff",
			itemBorderColor:"#f4eff4",
			itemBgMoveColor:"rgba(255, 240, 234, 1)",
			itemMoveColor:"red",
			itemColor:"#AAA",
			menuBarColor:"#FFF",
			menuContentColor:"#FFF"
...
Q6MenuBar.prototype.toString = function() {
	// menuBar对应的<div>
	var str = '<div style="color:' + this.colors[this.colorStyle].menuBarColor + ';" class = "menuBar" name="menuBar" onClick="' + this.obj + '.showMenu(event, this)">\n';
	// menuBar的标题<div>
...
Q6MenuBar.prototype.menu2Str = function(menuName){
	...
	// 菜单对应的<div>
	var str = '<div name="menu" class="menu" >\n';
	// 菜单标题对应的<div>，其中包含icon与text两部分。     onMouseenter="' + this.obj + '.onshow(this)" onMouseleave="' + this.obj + '.outhide(this)"
	str += '<div style="background-color:' + this.colors[this.colorStyle].menuBgColor + ';" name="menuTitle" class="menuTitle"><span name="menuTitleIcon" class="menuTitleIcon"><img src="' + icon + '"/></span><span class="menuTitleText">' + menuName + '</span></div>\n';
...
Q6MenuBar.prototype.item2Str = function(menuItem){
	return '<div style="background-color:' + this.colors[this.colorStyle].itemBgColor + '; color:' + this.colors[this.colorStyle].itemColor + ';" class="menuItem" onMouseMove="' + this.obj + '.itemMouseMove(this)" onMouseOut="' + this.obj + '.itemMouseOut(this)" onClick="skip(\'' + menuItem.url + '\', \'' + menuItem.frameName + '\')">' + menuItem.menuItemName + '</div>\n';
};
...
*/
//初始状态显示第一列菜单
Q6MenuBar.prototype.showoload = function(obj){
	obj.style.display = "block";
	var jianIcon = this.config.imgDir + this.icon.jianIcon;
	img.src = jianIcon;
	
};
Q6MenuBar.prototype.onshow = function(obj){
	obj.children[1].style.display = "block";
};
Q6MenuBar.prototype.outhide = function(obj){
